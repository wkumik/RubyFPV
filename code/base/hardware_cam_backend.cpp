...
// waybeam pre-v0.10.11 used /usr/bin/venc + /etc/venc.json. Set during detect.
static bool s_bLegacyVencNames = false;
...
t_hwcam_backend hwcam_be_detect()
{
   ...
   {
      s_Backend = ov;
      if ( s_Backend == HWCAM_BE_WAYBEAM )
         s_bLegacyVencNames = ( access("/usr/bin/waybeam", X_OK) != 0 ) && ( access("/usr/bin/venc", X_OK) == 0 );
      log_line("[HwCamBE] Backend from %s: %s%s", HWCAM_BE_OVERRIDE_FILE, hwcam_be_name(), s_bLegacyVencNames ? " (legacy venc paths)" : "");
      return s_Backend;
   }

   if ( access("/usr/bin/waybeam", X_OK) == 0 )
      s_Backend = HWCAM_BE_WAYBEAM;
   else if ( access("/usr/bin/venc", X_OK) == 0 )
   {
      s_Backend = HWCAM_BE_WAYBEAM;
      s_bLegacyVencNames = true;
   }
   else
      s_Backend = HWCAM_BE_MAJESTIC;

   log_line("[HwCamBE] Detected encoder backend: %s%s", hwcam_be_name(), s_bLegacyVencNames ? " (legacy venc paths)" : "");
   return s_Backend;
}
...
const char* hwcam_be_binary_path()
{
   if ( hwcam_be_get() != HWCAM_BE_WAYBEAM )
      return "/usr/bin/majestic";
   return s_bLegacyVencNames ? "/usr/bin/venc" : "/usr/bin/waybeam";
}
...
const char* hwcam_be_process_name()
{
   if ( hwcam_be_get() != HWCAM_BE_WAYBEAM )
      return "majestic";
   return s_bLegacyVencNames ? "venc" : "waybeam";
}

const char* hwcam_be_config_path()
{
   if ( hwcam_be_get() != HWCAM_BE_WAYBEAM )
      return "/etc/majestic.yaml";
   return s_bLegacyVencNames ? "/etc/venc.json" : "/etc/waybeam.json";
}

const char* hwcam_be_config_backup_path()
{
   if ( hwcam_be_get() != HWCAM_BE_WAYBEAM )
      return "/etc/majestic.yaml.org";
   return s_bLegacyVencNames ? "/etc/venc.json.org" : "/etc/waybeam.json.org";
}

const char* hwcam_be_kill9_cmd()
{
   if ( hwcam_be_get() != HWCAM_BE_WAYBEAM )
      return "killall -9 majestic";
   return s_bLegacyVencNames ? "killall -9 venc" : "killall -9 waybeam";
}
...
void hwcam_be_format_start_cmd(char* dst, int dst_sz, bool bLogToFile, const char* szLogPath)
{
   ...
   // Majestic needs '-s' flag to read stdin/config; waybeam takes no flags (reads its JSON config).
...
{
   if ( NULL == dst || dst_sz <= 0 || NULL == szKey || NULL == szValue )
      return;
   snprintf(dst, dst_sz, "curl -s 'localhost/api/v1/set?%s=%s'", szKey, szValue);
}

const char* hwcam_be_record_start_cmd()
{
   return "curl -s localhost/api/v1/record/start";
}

const char* hwcam_be_record_stop_cmd()
{
   return "curl -s localhost/api/v1/record/stop";
}

const char* hwcam_be_record_status_cmd()
{
   return "curl -s localhost/api/v1/record/status";
}
...
bool hwcam_be_supports_onboard_recording()
{
   return (hwcam_be_get() == HWCAM_BE_WAYBEAM);
}
